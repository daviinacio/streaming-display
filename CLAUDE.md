...
Each feature in `src/features/<name>/` follows the same shape: `components/`, `hooks/`, `lib/`, `types/`, and an `index.ts` barrel. Import from the barrel (`@/features/plugin`), not deep paths, when consuming from outside the feature.
...
- Each component has a `type` from `PLUGIN_TYPE_OPTIONS` ([constants/plugin-type.options.ts](src/features/plugin/constants/plugin-type.options.ts)): `player`, `source_handler`, `header_left`, `header_right`, `controls_left`, `controls_right`, `menu_item`. The type controls **where** the component mounts (`PluginMount` for URL-bound positions, `PluginMountGlobal` for app-wide positions like header/menu).
