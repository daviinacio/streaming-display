...
import { deserializePreferences } from "@/providers/preference-provider.tsx";
...
export function applyTheme(props?: ApplyThemeProps) {
  ...
  const preferences = deserializePreferences();

  // Theme colors
  colorPrimaryHsl = colorPrimaryHsl ?? preferences["color-primary"] ?? "142 72% 42%";
  ...
  // Theme schema
  isDarkMode = isDarkMode ?? (function(){
